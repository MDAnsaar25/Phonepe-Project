...
def agg_insurance():
    ...
    path="C:/Users/Admin/OneDrive/Desktop/Phonepe Project/phonepe-project/pulse/data/aggregated/insurance/country/india/state/"
...
def agg_trans():
    ...
    cursor=connection.cursor()
    path="C:/Users/Admin/OneDrive/Desktop/Phonepe Project/phonepe-project/pulse/data/aggregated/transaction/country/india/state/"
    path_list=os.listdir(path)
    column={"state":[],"year":[],"quarter":[],"transaction_name":[],"count":[],"amount":[]}
    for state in path_list:
        state_link=path+state+"/"
...
def agg_user():
    ...
    path2="C:/Users/Admin/OneDrive/Desktop/Phonepe Project/phonepe-project/pulse/data/aggregated/user/country/india/state/"
...
def map_insurance():
    ...
    path="C:/Users/Admin/OneDrive/Desktop/Phonepe Project/phonepe-project/pulse/data/map/insurance/hover/country/india/state/"
    path_list=os.listdir(path)
...
def map_trans():
    ...
    path="C:/Users/Admin/OneDrive/Desktop/Phonepe Project/phonepe-project/pulse/data/map/transaction/hover/country/india/state/"
    path_list=os.listdir(path)
    column={"state":[],"year":[],"quarter":[],"district_name":[],"count":[],"amount":[]}
    for state in path_list:
        state_link=path+state+"/"
...
def map_users():
    ...
    cursor=connection.cursor()
    path="C:/Users/Admin/OneDrive/Desktop/Phonepe Project/phonepe-project/pulse/data/map/user/hover/country/india/state/"
    path_list=os.listdir(path)
...
def top_insurance():
    ...
    path="C:/Users/Admin/OneDrive/Desktop/Phonepe Project/phonepe-project/pulse/data/top/insurance/country/india/state/"
    path_list=os.listdir(path)
    column={"state":[],"year":[],"quarter":[],"district_name":[],"count":[],"amount":[]}
    for state in path_list:
        state_link=path+state+"/"
...
def top_trans():
    ...
    path="C:/Users/Admin/OneDrive/Desktop/Phonepe Project/phonepe-project/pulse/data/top/transaction/country/india/state/"
    path_list=os.listdir(path)
    column={"state":[],"year":[],"quarter":[],"district_name":[],"count":[],"amount":[]}
    for state in path_list:
        state_link=path+state+"/"
...
def top_users():
    ...
    cursor=connection.cursor()
    path="C:/Users/Admin/OneDrive/Desktop/Phonepe Project/phonepe-project/pulse/data/top/user/country/india/state/"
    path_list=os.listdir(path)
...
#DASHBOARD-TOTAL TRANSACTION(COUNT)
def total_trans():
    sql_query="SELECT SUM(Count) as total_transaction FROM Aggregated_transaction"
...
def total_payment_value():
    sql_query="SELECT SUM(Amount) as total_amount from Aggregated_transaction"
    cursor.execute(sql_query)
    res=cursor.fetchone()
    res_in_value=res[0]
    res_in_float=float(res_in_value)
...
def avg_trans_value():
    sql_query="Select AVG(Amount) as avg_amount from Aggregated_transaction"
    cursor.execute(sql_query)
    res=cursor.fetchone()
    res_in_value=res[0]
    res_in_float=float(res_in_value)
...
def categories():
    sql_query='''SELECT Transaction_Name, SUM(Amount) AS total_amount 
    ...
    for row in results:
        ...
        st.subheader(f":orange[{transaction_type}]: {total_amount_in_crores} Cr")      
    
#DASHBOARD- TOP 10 STATES(TRAN_AMOUNT)
def top_ten_states():
    sql_query=('''SELECT State, SUM(Amount) AS Total_Amount
                    FROM aggregated_transaction
                    GROUP BY State
...
if selected == "Transactions":
    ...
    with col0:
        ...
        op1 = st.radio(
        "Choose Type",
        [":rainbow[State Wise]",":rainbow[District Wise]",":rainbow[Transaction Type]",
         ":rainbow[Year Wise Data]",":rainbow[Quarter Wise Data]"])
    if op1 == ":rainbow[Transaction Type]":
        ...
        with col1:
            cursor.execute(f"""SELECT State, Transaction_Name, SUM(Amount) AS Total_Amount                   
                            FROM aggregated_transaction 
                            WHERE Year = {user_year1}
                            GROUP BY State, Transaction_Name;""")
            df_amount = pd.DataFrame(cursor.fetchall(),
                                columns=["State", "Transaction_Name", "Total_Amount"])
            fig_amount = px.pie(df_amount, names="Transaction_Name", values="Total_Amount", hover_data=['State'],
                                labels={'Total_Amount': 'Total_Amount'}, hole=0.4,
                                title="Amount Spent on Various Categories", width=600, height=600)
            st.plotly_chart(fig_amount, use_container_width=True)
        
        with col2:
            cursor.execute(f"""SELECT State, Transaction_Name, SUM(Count) AS Total_Count                   
                            FROM aggregated_transaction 
                            WHERE Year = {user_year1}
                            GROUP BY State, Transaction_Name;""")
            df_count = pd.DataFrame(cursor.fetchall(),
                                columns=["State", "Transaction_Name", "Total_Count"])
            fig_count = px.pie(df_count, names="Transaction_Name", values="Total_Count", hover_data=['State'],
    ...
    elif op1 == ":rainbow[District Wise]":
        col0,col01=st.columns(2)
        with col0:
            cursor.execute("SELECT DISTINCT State FROM map_transaction")
            list_of_states = [row[0] for row in cursor.fetchall()]
            selected_state = st.selectbox("Choose One", list_of_states, index=0) 
        col1,col2=st.columns(2)
        with col1:
            cursor.execute(f"""SELECT District_Name, SUM(Count) AS Total_Count
                                FROM map_transaction
                                WHERE State = '{selected_state}'
                                GROUP BY District_Name;""")
            district_data = cursor.fetchall()
            district_df = pd.DataFrame(district_data, columns=["District", "Count"])
            fig = px.bar(district_df, x='District', y='Count', title='Total Transaction Count',
                        height=600)
            st.plotly_chart(fig, use_container_width=True)           
        with col2:
            cursor.execute(f"""SELECT District_Name, SUM(Amount) AS Total_Amount
                                FROM map_transaction
                                WHERE State = '{selected_state}'
                                GROUP BY District_Name;""")
            district_data = cursor.fetchall()
            district_df = pd.DataFrame(district_data, columns=["District", "Amount"])
            fig = px.bar(district_df, x='District', y='Amount', title='Total Transaction Amount',
                        height=600)
            st.plotly_chart(fig, use_container_width=True)
...
if selected == "Insurance":
    ...
    genre = st.radio(
    "Choose Type",
    [":rainbow[State Wise]",':rainbow[District Wise]',":rainbow[Year Wise]",":rainbow[Quarter Wise]"])
    ...
    if genre == ':rainbow[District Wise]':
        col0,col01=st.columns(2)
        with col0:
            cursor.execute("SELECT DISTINCT State FROM map_insurance")
            list_of_states = [row[0] for row in cursor.fetchall()]
            selected_state = st.selectbox("Choose One", list_of_states, index=0) 
        col1,col2=st.columns(2)
        with col1:
            ...
            district_data = cursor.fetchall()
            district_df = pd.DataFrame(district_data, columns=["District", "Count"])
            fig = px.bar(district_df, x='District', y='Count', title='Total Insurance Purchased',
                        height=600)
            st.plotly_chart(fig, use_container_width=True)           
        with col2:
            cursor.execute(f"""SELECT District_Name, SUM(Amount) AS Total_Amount
            ...
            district_data = cursor.fetchall()
            district_df = pd.DataFrame(district_data, columns=["District", "Amount"])
            fig = px.bar(district_df, x='District', y='Amount', title='Total Insurance Transaction',
                        height=600)
            st.plotly_chart(fig, use_container_width=True)
...
if selected == "Users":
    ...
    option = st.radio(
    "Choose Type",
    [":rainbow[State Wise]",":rainbow[District Wise]",":rainbow[Brand Wise]",":rainbow[Quarter and Year Wise]"])
    if option == ":rainbow[State Wise]":
        col91,col92=st.columns(2)
        with col91:
            user_year1 = st.slider('Choose the Year',min_value=2020,max_value=2023)
        ins_map1,ins_map2 = agg_users(user_year1)
        col9,col10=st.columns(2)
        col91,col92=st.columns(2)
        with col9:
            st.plotly_chart(ins_map1)
    ...
    if option == ":rainbow[Brand Wise]":
        ...
        with col1:
            ...
            df_amount = pd.DataFrame(cursor.fetchall(),
                                columns=["State", "Brand_Name", "Count"])
            fig_amount = px.pie(df_amount, names="Brand_Name", values="Count", hover_data=['State'],
                                labels={'Count': 'Count'}, hole=0.4,
...
if selected == "Data Analysis":
    ...
    if question == "1. Top 10 States in Terms of Insurance Penetration":
        ...
        sql_query=("""SELECT Transaction_Name,SUM(Amount) AS Total_Amount_Spent
                    FROM aggregated_transaction GROUP BY Transaction_Name ORDER BY Total_Amount_Spent DESC
                    LIMIT 1;""")
        cursor.execute(sql_query)
        res=cursor.fetchall()
        ...
        sql_query=('''SELECT Year,SUM(Count) AS Total_Transaction_Count
        ...
        sql_query=('''SELECT Transaction_Name,SUM(Count) AS Total_Volume
                        FROM aggregated_transaction
                        WHERE State = 'Andaman and Nicobar Islands'
                        GROUP BY Transaction_Name ORDER BY Total_Volume ASC LIMIT 1;''')
        cursor.execute(sql_query)
        res = cursor.fetchall()
        ...
        sql_query = '''SELECT Transaction_Name, SUM(Count) AS Total_Count
               FROM aggregated_transaction
               GROUP BY Transaction_Name;'''
        cursor.execute(sql_query)
        res = cursor.fetchall()
        df = pd.DataFrame(res, columns=['Transaction_Name', 'Total_Count'])
        st.line_chart(df.set_index('Transaction_Name'))
        fig = px.line(df, x='Transaction_Name', y='Total_Count', 
